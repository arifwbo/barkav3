  ...
  if ( page <= parseInt(total_page) ) {
    ...
    $.post( _BASE_URL + 'public/employee_directory/get_employees', data, function( response ) {
      ...
      for (var z in rows) {
        const row = rows[ z ];
        const template = `
        <div class="profile-employees group animate-fade-in">
            ...
            <div class="space-y-3 w-full">
              ...
              <dl class="grid grid-cols-1 gap-2 text-sm">
                <div class="bg-gray-50 p-3 rounded-lg">
                  <dt class="text-muted text-xs uppercase tracking-wide">NIK</dt>
                  <dd class="font-medium text-gray-800">${row.nik}</dd>
                </div>
                <div class="bg-gray-50 p-3 rounded-lg">
                ...
                <div class="bg-gray-50 p-3 rounded-lg">
                  <dt class="text-muted text-xs uppercase tracking-wide">Jenis GTK</dt>
                  <dd class="font-medium text-gray-800">${row.employment_type}</dd>
                </div>
              </dl>
            </div>
          </div>
...
<main class="container space-y-5 my-5 flex-1">
  <div class="space-y-4">
        ...
        <div class="profile-employees group animate-fade-in">
          <div class="flex flex-col items-center text-center space-y-4">
            <div class="relative">
              ...
              echo '<img src="' . base_url('media_library/employees/'.$photo).'" class="w-24 h-24 lg:w-32 lg:h-32 object-cover rounded-full border-4 border-white shadow-lg group-hover:scale-105 transition-transform duration-300" loading="lazy">';
              ?>
              <div class="absolute -bottom-2 -right-2 w-8 h-8 bg-gradient-to-r from-primary-color to-accent-color rounded-full flex items-center justify-center">
                <i class="fa fa-briefcase text-white text-sm"></i>
              </div>
    ...
    <div class="text-center">
      <button type="button" onclick="get_employees()" class="btn more-employees">
